...
|---|---|---|---|---|---|
| Week 1 | Independent baselines | Define pickup identifiers and QR/web flow | Establish camera availability and image-acquisition rules; verify Pi and real A2M8 scans | Preserve the verified UART baseline; restore and test original radio control with wheels raised | QR request prototype, camera/data rules, real scan evidence and safe manual-control evidence |
| Week 2 | Interfaces and real data | Define pickup-task states and prepare planning software reproduction | Establish camera interface/mounting; acquire first labelled image sessions; save scan recordings | Measure isolated actuator-control signals; prepare STM32 PWM, timeout and sensor interfaces | A/B pickup, pose and obstacle interfaces recorded; high-level/low-level vehicle interface recorded |
| Week 3 | Low-level control and perception preparation | Prepare global/local planning and recorded-data replay | Continue image acquisition across actual viewpoints; prepare AI training and lidar-mapping pipelines | Verify STM32 neutral, steering, low-speed forward, stop and reverse with the vehicle secured; acquire IMU and SHARP data | Safe low-level control and reproducible camera/lidar/sensor acquisition |
| Week 4 | Motion feedback and first map | Implement the pickup-task state machine without claiming autonomous motion | Split visual data by acquisition session; build and save the first real-vehicle lidar map | Integrate photointerrupter counting, speed magnitude, inferred direction limits and calibrated steering feedback | Reusable map, dataset split and motion feedback with explicit limitations |
| Week 5 | Separate localization paths | Prepare route planning against the saved map and TT-02 constraints | Train and validate visual-place recognition on held-out runs; run lidar localization using C's time-aligned feedback | Provide timestamped speed/IMU/status streams and diagnose mounting/axis issues | Visual region output and continuous lidar-based pose tested separately |
| Week 6 | Fused localization and navigation input | Consume pose, obstacle and station-pose interfaces; prepare goal planning | Run camera inference on the Pi; constrain initialization, check consistency or recover lidar localization using validated visual output | Support repeated starts and motion-feedback diagnosis | Repeated `(x, y, yaw)` localization reaches A through a recorded interface |
| Week 7 | Fixed pickup targets | Generate final QR material, handle requests and convert accepted tasks into goal lookups | Measure each pickup stopping pose `(x, y, yaw)` on the map and maintain identifier-to-pose data | Integrate target speed/steering and vehicle-status feedback | One pickup request reaches one mapped stopping pose through recorded interfaces |
| Week 8 | Path following and arrival | Plan and follow a route respecting TT-02 dimensions, steering and turning radius; display task states | Maintain pose quality and AI/lidar diagnostics during motion | Tune low-speed control, feedback, stopping and communication timeout | One obstacle-free pickup task is reproducible and arrival error is recorded |
| Week 9 | Obstacle handling | Build the live obstacle costmap; detour/replan when feasible; display blocked and resumed states | Supply validated lidar obstacle data and check localization stability around obstacles | Implement calibrated close-range SHARP protection, stop priority, takeover and recovery | Detection, stopping, detouring and blockage are accepted separately |
| Week 10 | Complete taxi workflow | Connect QR request, route execution, obstacle handling, arrival and user status | Connect visual constraint, lidar pose and localization-health output to the workflow | Record fault states, logs and reconnection behavior | Complete fixed-pickup workflow under the team's documented test scope |
| Week 11 | Repeated tests and fixes | Analyze QR/request, planning, following, detour and user-flow failures | Analyze visual-model, lidar and fused-localization failures | Analyze control, communication, speed, IMU and protection failures | Record trial outcomes and regression checks; stop expanding feature scope |
| Week 12 | Freeze, report and demonstrate | QR/web, task state, planning/obstacle configurations and contribution record | Dataset, model, maps, localization configurations and demonstration evidence | Interface, firmware, calibration, control and recovery material | Freeze the demonstration version; complete backups, startup instructions, report and rehearsal |
