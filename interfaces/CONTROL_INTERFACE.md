...
| Communication parameters and message format | STM32 standalone configuration: 115200 bit/s, 8 data bits, no parity and one stop bit. The diagnostic request is ASCII `PING\n`; the response is ASCII `PONG\n`. The receiver ignores `\r`, so `PING\r\n` is accepted. The current implementation sends no response for an unrecognized line. Control and feedback messages remain to be agreed with B. | Standalone protocol test on 2026-09-22; control protocol requires B/C agreement |
...
| Corresponding firmware/host versions and test records | STM32 interrupt-reception firmware: `3f10c88`. Standalone evidence: `tests/records/2026-09-22_stm32_usart2_interrupt.md` and `tests/records/2026-09-22_stm32_ping_pong_protocol.md`. Raspberry Pi version and integration record remain pending. | Repository commits and actual integration records from both sides |
